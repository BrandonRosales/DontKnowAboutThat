...
bool Client::sendChatString(string &_string) {
...
bool Client::sendPlayerName(string _string) {
	if (!sendPacketType(P_NameRequest)) {//sends packet type: Chat Message, if 
		return false; // return false if could not send packet
	}
	int bufferLength = _string.size(); // finds string buffer length
	if (!sendInt(bufferLength)) { // sends length of string buffer 
...
bool Client::sendCardsSubmition(string &_string) {
	if (!sendPacketType(P_CardsSubmition)) {
		return false; // return false if could not send packet
	}
	int bufferLength = _string.size(); // finds string buffer length
	if (!sendInt(bufferLength)) { // sends length of string buffer 
...
bool Client::sendYesNoResponse(string &_string) {
	if (!sendPacketType(P_YesNoResponse)) {
		return false; // return false if could not send packet
	}
	int bufferLength = _string.size(); // finds string buffer length
	if (!sendInt(bufferLength)) { // sends length of string buffer 
