...
#include "DKATGame.h"
  ...
  */
int DKATGame::numberOfPlayersWithNames = 0;
Player DKATGame::players[8] = { Player() };
CardPile DKATGame::cardPile;
string DKATGame::activeCardValue = "0";
...
DKATGame::DKATGame(int numberOfPlayers, Server& server) : playerNumber(numberOfPlayers), server(server) {
	...
	if (numberOfPlayers >= 3) {
		...
		cout << message << endl;
		server.sendChatStringToAll(message); // WILL NOT BE READ ALLOWED JUST YET TO PLAYERS
		...
		server.sendPacketTypeToAll(P_NameRequest);
		//wait for name replies
		while (numberOfPlayersWithNames < numberOfPlayers) {
			Sleep(1000);
		}
		//sends names to everyone
		server.sendNamesToPlayersAll();
...
void DKATGame::playGame() {
	...
	stringstream ss;
	string message;
	ss << "The winner of the game is "<< players[playersTurn].name;
	message = ss.str();
	server.sendChatStringToAll(message);
	server.updateGame();
	cout << message << endl;
...
void DKATGame::displayActiveNumberAndPlayer(int activeCardNumber, int activePlayer) {
	...
	ss3 << "It is " << players[activePlayer].name << "'s turn";
...
void DKATGame::playerSubmitsCards(int activeCardNumber, int activePlayer) {
	...
	if (onePlayerRun) {
		...
		activatePlayersTurn(activePlayer, cardsToSubmit); // this activates the part where the user is able to submit
...
bool DKATGame::processingSubmitCards(int numberActivated, Player &player, int playerID, string cardsToSubmit) {
	...
	while ((pos = cardsToSubmit.find(",")) != std::string::npos) { //Stops so it doesn't go pass EOF 
		if (numberOfCardsSubmitted == 3) { //or if there have been 3 tokens submitted already.
			...
			server.sendChatString(playerID, message);
			return false;
		}
...
void DKATGame::playerResponses(int activeCardNumber, int activePlayer) {
	...
	for (int i = activePlayer, j = 0; j != playerNumber && (!aPlayerChallenged); i++, j++) {
		if (i==playerNumber) {
			i = 0;
		}
		if (activePlayer != i) { // only the other players to respond
			while (!onePlayerRun || (playerOneRunPlayerNumber != activePlayer&&playerOneRunPlayerNumber == i)) { //Will run if one player mode is off, or...
				...
				server.sendResponseNotification(i, activePlayer, lastNumberOfCardsSubmitted);

				cout << "Waiting for response from: " << i << endl;
				...
				if (response == "y") {
					...
					ss.clear();
					ss.str(string());
					ss << players[i].name << " doesn't think " << players[activePlayer].name << " is bluffing.";
					message = ss.str();
					server.sendChatStringToAll(message);
					break;
				}
					...
					server.sendChatString(i, message);
				}
...
bool DKATGame::challengePlayer(int activeCardNumber, int activePlayer, int challengingPlayer) {
	stringstream ss;
	string message;
	ss << players[challengingPlayer].name << " thinks " << players[activePlayer].name << " is bluffing.";
	message = ss.str();
	server.sendChatStringToAll(message);
	for (int i = 0; i < lastNumberOfCardsSubmitted; i++) {
		...
		ss << "Comparing: " << activeCardNumber << " to value " << Card::ALLCARDS->findCardValue(lastCardsSubmitted[i]);
		message = ss.str();
		server.sendChatStringToAll(message);
		if (Card::ALLCARDS->findCardValue(lastCardsSubmitted[i]) != activeCardNumber) {
			...
			ss << players[activePlayer].name << " got the pile!";
			message = ss.str();
			server.sendChatStringToAll(message);
	...
	ss << players[challengingPlayer].name << " added the pile to their hand!";
	message = ss.str();
	server.sendChatStringToAll(message);
...
bool DKATGame::activatePlayersTurn(int activePlayer, string &cardsToSubmit) {
	//push packets here
	server.sendActivationAndPush(activePlayer);
