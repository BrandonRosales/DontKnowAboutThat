#include "Server.h"
#include "DKATGame.h" // crashes the program when in server - temporary for now
...
//void Server::attachGame(DKATGame &_game) {
//	game = &_game;
//}
...
bool Server::listenForNewConnection() {
	...
	if (newConnection == INVALID_SOCKET) {
		...
		sendChatString(totalConnections, string("Connected!"));
...
bool Server::processPacket(int ID, Packet packetType) {
	...
	case P_NameRequest:
	{
		...
		if (!getString(ID, playerName)) { //string to be stored in message
			cout << "11" << endl;
			return false; // if we did not properly get the string
		}

		DKATGame::players[ID].name = playerName;
		cout << "Name received by " << ID << " they're name is: " << playerName << endl;
		DKATGame::numberOfPlayersWithNames++;
		break;
	}
	case P_CardsSubmition:
	{
		getString(ID, stringResponse); // card submission stored here
	...
	{
		getString(ID, stringResponse); // yes no submission stored here
		setListenID = -1; // sets the listen id back to -1
		break;
	}
	default:
...
void Server::updateGame() {
	sendCardPileAmountToAll();
	sendCardAmountsToAll();
	sendCardsToAll();
	sendActiveCardValueToAll();
}